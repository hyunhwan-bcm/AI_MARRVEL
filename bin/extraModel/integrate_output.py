...
def integrate_output(prj_folder, data_folder, sample_id):
    ...
    default_raw_pred_dict = dict(
        zip(default_pred.index.tolist(), default_pred.predict.tolist())
    )
    default_conf_dict = dict(
        zip(default_pred.index.tolist(), default_pred["confidence"].tolist())
    )
    default_conf_lvl_dict = dict(
        zip(default_pred.index.tolist(), default_pred["confidence level"].tolist())
    )
    default_rank_dict = dict(
        zip(default_pred.index.tolist(), default_pred.ranking.tolist())
    )
    ...
    expanded_df["confidence level (default)"] = expanded_df.index.map(
        default_conf_lvl_dict
    )
    ...
    nd_conf_lvl_dict = dict(
        zip(nd_pred.index.tolist(), nd_pred["confidence level"].tolist())
    )
    ...
    if os.path.exists(recessive_fn):
        ...
        recessive_pred_dict = dict(
            zip(recessive_pred.index.tolist(), recessive_pred.predict.tolist())
        )
        recessive_conf_dict = dict(
            zip(recessive_pred.index.tolist(), recessive_pred["confidence"].tolist())
        )
        recessive_rank_dict = dict(
            zip(recessive_pred.index.tolist(), recessive_pred.ranking.tolist())
        )
        recessive_other_var = dict(
            zip(recessive_pred.index.tolist(), recessive_pred.var2.tolist())
        )
        recessive_conf_lvl_dict = dict(
            zip(
                recessive_pred.index.tolist(),
                recessive_pred["confidence level"].tolist(),
            )
        )
    ...
    if os.path.exists(nd_recessive_fn):
        ...
        nd_recessive_pred_dict = dict(
            zip(nd_recessive_pred.index.tolist(), nd_recessive_pred.predict.tolist())
        )
        nd_recessive_conf_dict = dict(
            zip(
                nd_recessive_pred.index.tolist(),
                nd_recessive_pred["confidence"].tolist(),
            )
        )
        nd_recessive_rank_dict = dict(
            zip(nd_recessive_pred.index.tolist(), nd_recessive_pred.ranking.tolist())
        )
        nd_recessive_conf_lvl_dict = dict(
            zip(
                nd_recessive_pred.index.tolist(),
                nd_recessive_pred["confidence level"].tolist(),
            )
        )
        nd_recessive_other_var = dict(
            zip(nd_recessive_pred.index.tolist(), nd_recessive_pred.var2.tolist())
        )
